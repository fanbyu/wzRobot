...
#define MAXCM 500             // 5m
#define MEASURE_TIMEOUT       (MAXCM * 58)
...
uint16_t RgbUltrasonic::GetUltrasonicFrontDistance()
{
  ...
  if (_measureFlag == true) {
    ...
    Time_Echo_us = pulseIn(SignalPin, HIGH, MEASURE_TIMEOUT * 2);
    
    if ((Time_Echo_us < MEASURE_TIMEOUT) && (Time_Echo_us > 1)) {
...
void RgbUltrasonic::SetRgbEffect(E_RGB_INDEX index, long Color_R, long Color_G, long Color_B, uint8_t effect)
{
	long Color = (Color_R << 16) | (Color_G << 8) | (Color_B);
	SetRgbEffect(index, Color, effect);
}
