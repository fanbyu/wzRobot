/**
 * 程序自动判断兼容 V1 与 V2 版本模块
 * V1 版本模块 与 V2 版本模块 模拟值 范围不同，且传感器编号相反，
*/
...
void InfraredTracking::Init() {
    ...
    if(FIFO[0] <=3 && FIFO[1] > 3) { // 在默认的V1读取方式下在高 8 位有数据，说明模块是 V2 版本
        version = 2;
...
bool InfraredTracking::ReadFIFO()
{
    uint8_t result;
...
void InfraredTracking::GetRawDat()
{
    ...
    if(version >= 2) {
        ...
        ir_track5 = (FIFO[0] & 0xFF) | ((FIFO[1] & 0xFF) << 8 );
        ir_track4 = (FIFO[2] & 0xFF) | ((FIFO[3] & 0xFF) << 8 );
        ir_track3 = (FIFO[4] & 0xFF) | ((FIFO[5] & 0xFF) << 8 );
        ir_track2 = (FIFO[6] & 0xFF) | ((FIFO[7] & 0xFF) << 8 );
        ir_track1 = (FIFO[8] & 0xFF) | ((FIFO[9] & 0xFF) << 8 );
    } else {
