...
# 2. Add Sentry2 namespace at the end (before final })
sentry2_code = '''
...
# Append sentry2 before the last closing brace
content = content.rstrip()
if content.endswith('}'):
    content = content[:-1] + sentry2_code + '\n}'

# 3. Add per-block colors to five_tracker_V3 and motor_driver blocks
color_map = {
    'five_tracker_V3': '#409EFF',
    'motor_driver': '#E6A23C',
}

for ns_name, color in color_map.items():
    # Find the namespace block
    ns_start = content.find('namespace ' + ns_name)
    if ns_start == -1:
        continue
    
    # Find end: next namespace or end of file
    remaining = content[ns_start + 10:]
    next_pos = len(remaining)
    for other_ns in ['five_tracker_V3', 'motor_driver', 'sentry2']:
        if other_ns == ns_name:
            continue
        pos = remaining.find('namespace ' + other_ns)
        if pos != -1 and pos < next_pos:
            next_pos = pos
    
    ns_block = content[ns_start:ns_start + 10 + next_pos]
    
    # Add color to blockType lines that don't have color=
    def add_color(match, c=color):
        return match.group(1) + 'color="' + c + '" blockType='
    
    new_block = re.sub(r'(    //% block="[^"]*"\s+)blockType=', add_color, ns_block)
    
    if new_block != ns_block:
        added = new_block.count('color="' + color + '"') - ns_block.count('color="' + color + '"')
        print(ns_name + ': +' + str(added) + ' colored blocks')
    
    content = content[:ns_start] + new_block + content[ns_start + 10 + next_pos:]

with open('arduinoC/main.ts', 'w', encoding='utf-8') as f:
    f.write(content)

total = len(re.findall(r'color="#[0-9A-Fa-f]+" blockType=', content))
print('Total per-block colored: ' + str(total))
print('Done!')
